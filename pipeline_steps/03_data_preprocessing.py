...
class ClinicalTrialPreprocessor:
    ...
    def preprocess(self, df):
        ...
        # Clean and extract interventions for downstream processing
        if 'interventions_names' in df.columns:
            df['intervention_names_clean'] = (
                df['interventions_names'].fillna('').apply(
                    lambda x: '|'.join(sorted(
                        set(i.strip().lower() for i in re.split(r'\||,', x) if i.strip())
                    ))
                )
            )
        else:
        ...
        print(f"✅ Saved unique drug names for fingerprinting.")
